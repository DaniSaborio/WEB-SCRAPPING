...
from datetime import datetime # <<< --- Importa datetime para nombres de archivo
...
# --- Directorio de exportación (Asegúrate de que esta ruta sea correcta para tu proyecto) ---
# Asumiendo que 'scrapper.py' está en 'WEB-SCRAPPING/scraper/'
# y quieres guardar en 'WEB-SCRAPPING/server_data/exports/'
EXPORT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'server_data', 'exports')
...
# --- NUEVA FUNCIÓN PARA GUARDAR DATOS EN CSV ---
def save_data_to_csv(data, directory, filename_prefix="steam_discounts"):
    ...
    if not data:
        logging.warning(f"No hay datos para guardar en {file_path}. No se creará el archivo CSV.")
        return None # Devuelve None si no se crea el archivo

    # Obtener los nombres de las columnas del primer diccionario (asumiendo que todos tienen las mismas claves)
    ...
    try:
        ...
        return file_path # Devuelve la ruta del archivo creado
    except IOError as e:
